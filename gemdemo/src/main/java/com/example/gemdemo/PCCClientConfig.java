...
public class PCCClientConfig {
	...
	@Bean
    public ClientRegionFactoryBean<String, Greeting> customerRegion (@Autowired ClientCache gemfireCache) {

        ClientRegionFactoryBean<String, Greeting> greetRegion =
        		new ClientRegionFactoryBean<String, Greeting>();
        greetRegion.setCache(gemfireCache);
        greetRegion.setClose(false);
        greetRegion.setShortcut(ClientRegionShortcut.PROXY);
        greetRegion.setLookupEnabled(true);
        return greetRegion;
    }
